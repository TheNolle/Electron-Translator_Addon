...
function initLang() {
    ...
    if (localStorage.getItem('lang') && localStorage.getItem('lang-file')) { // if have localstorage
        ...
        fs.readFile('./translator/langs/' + localStorage.getItem("lang-file"), 'utf8', (err, data) => { // Read the lang file selectioned
            ...
            traduction("LangsTitlePopup", file.LangsTitlePopup);
            traduction("ExampleTitle", file.ExampleTitle);
            traduction("ExampleText", file.ExampleText);

            traduction("IncorporationFooter", file.IncorporationFooter);
            traduction("AboutFooter", file.AboutFooter);
            traduction("DataUsageFooter", file.DataUsageFooter);
            traduction("HowToUseFooter", file.HowToUseFooter);
            traduction("ChangeLanguageFooter", file.ChangeLanguageFooter);

            traduction("SocialLinks", file.SocialLinks);
            traduction("NewsBody", file.NewsBody);

            traduction("NewsTitle1", file.NewsTitle1);
            traduction("1NewsText1", file.NewsText11);
            traduction("2NewsText1", file.NewsText12);
            traduction("ReadTheArticle", file.ReadTheArticle);
        })
    }
}
